...
**当前 main 提交**：`aa992dc fix: use policy queries for review sentiment`

**试运行周期**：1-2 个交易日

**目标**：让稳米/洋米在真实查数场景里试用 `ym_stock_data.v2.resolve()`，验证 intent 入口、字段口径、新鲜度标记和问财批量 query 是否有用。试运行期间不接生产消费端。
...
for intent in ["realtime_market", "review_sentiment"]:
    r = resolve(intent)
    print("\n==", intent, "==")
    print(r["_meta"])
...
- 和原来 `fetch("index")` / `fetch("iwencai")` 直接查有没有明显差异。
...
1. `scripts/compare_v1_v2.py`
   - 对比 v1 `fetch()` 和 v2 `resolve()` 的关键字段。
   - 先覆盖 `realtime_market`、`review_sentiment`。

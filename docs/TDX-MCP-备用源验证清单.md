...
- 每次 session：`initialize` → `tools/list` 本次请求 capability schema gate → `tools/call`。
- 其它 capability 缺失或 schema drift 不得连带禁用本次能力；完整六项健康只能由后续 smoke/acceptance 分项验收。
...
| `tools/list` | 本次请求 capability 缺失或 schema drift 不进入 `tools/call`；无关能力不连坐 |
...
4. 只读 probe 必须观察到 SDK `initialize`、本次 capability 的 `tools/list` schema gate 与

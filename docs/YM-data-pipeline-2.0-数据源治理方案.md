...
- 只实现 MVP：`realtime_market`、`sector_index`、`stock_snapshot`、`stock_kline`、`review_sentiment` 五个 intent，40 个关键字段策略，标准 `_meta`。
...
└── v2/
    ...
    └── policies/
        └── fields.json      # 40 个关键字段的主源、备源、口径、频率
...
- `resolve("sector_index")`：直连 `sources.ths_industry.fetch_sector_index()`，只使用同花顺 881xxx 行业口径。
- `resolve("stock_snapshot")`：直连 `sources.pytdx.fetch_quotes()`，补个股行情 `_meta`。
- `resolve("stock_kline")`：直连 `sources.pytdx.fetch_kline()`，补 K 线、均线、`period` 和 `count` 截断 `_meta`。
- `resolve("review_sentiment")`：按字段策略批量执行问财 query，保留原始 query，并在顶层输出 `涨停收益均值`、`红盘率`、`炸板率`、`最高板` 等聚合字段。
- `fields.json`：覆盖 40 个关键字段，含 `data_scope`、`trade_usage`、`staleness_sec`、`rate_class`。
...
resolve("sector_index", codes=["881124"])
resolve("sector_index", names=["消费电子", "通信设备"])
resolve("stock_snapshot", codes=["002475", "002281"])
resolve("stock_kline", code="002475", period="15m", count=20)
...
- v1/v2 compare 覆盖 `realtime_market`、`sector_index`、`stock_snapshot`、`stock_kline`、`review_sentiment`。
...
- 先实现 `realtime_market`、`sector_index`、`stock_snapshot`、`stock_kline` 和 `review_sentiment`，不急于实现全量 intent。
...
第一优先级：
1. 数据入口审计。
2. v2.0 MVP 闭环：`realtime_market`、`sector_index`、`stock_snapshot`、`stock_kline`、`review_sentiment`、40 个字段、标准 `_meta`。
...
5. 新增 `ym_stock_data/v2/policies/fields.json`，覆盖 40 个最关键字段。
...
- v2.0 MVP 只实现 realtime_market、sector_index、stock_snapshot、stock_kline 和 review_sentiment；不要扩成全量平台。

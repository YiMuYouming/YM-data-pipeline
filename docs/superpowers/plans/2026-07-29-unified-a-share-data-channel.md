...
| `review_sentiment` with explicit query | `iwencai_openapi` → `pywencai` → `tdx_screener` → `wind_screener`; experimental `pytdx_screener` is explicit-only and never appended | stock-screen rows only; final empty requires all four providers in the canonical route to be valid empty |
...
> **2026-07-30 five-source-capability amendment (superseded):** The formal window supersedes the unpublished 1.2 draft with acceptance 1.3, smoke schema 2, baseline `five-source-capabilities-v1`, and exactly 21 ordered cases. The original 10 core cases and the PyTDX direct case remain. Direct registry probes separately require non-empty success from OpenAPI, pywencai, all six TDX capabilities, all three Wind boundaries, and PyTDX. A controlled canonical-router case injects the first four outcomes and permits only the final PyTDX provider to be live; exact attempt order and `origin=injected|live` are gated. `observation_day_count` and consecutive `pass_day_count` are independent. Gate failure writes no acceptance receipt; a missed previous official trading date, baseline change, or HEAD change closes the epoch and the next pass restarts Day 1. Only five consecutive gate passes complete the window. Acceptance 1.0/1.1 remain read-only; the unpublished 1.2 baseline never counts. A live smoke neither authorizes OAuth login nor schedules later runs automatically.

> **2026-08-01 four-source simplification amendment (current):** This supersedes the 2026-07-30 five-source capability policy. The formal managed source set is WenCai OpenAPI, portable pywencai, TDX owned OAuth, and the official Wind CLI. The canonical natural-language route is always `iwencai_openapi` → `pywencai` → `tdx_screener` → `wind_screener`. Zero-auth PyTDX remains an experimental explicit provider for diagnostics and development, but is not appended to public `query()`, is not called live by formal smoke, and cannot pass or fail the source gate. Acceptance remains schema 1.3 / smoke schema 2 / exactly 21 ordered cases, with baseline `four-source-capabilities-v1`: the former PyTDX direct position now records offline provider state, and the controlled route injects OpenAPI plus pywencai failures before requiring a live TDX screener success. The baseline change restarts the five-official-trading-day window. TDX may use the user-authorized credential copy already imported from WorkBuddy with `imported_from=workbuddy`; runtime code must not scan or continuously synchronize WorkBuddy storage. The validated WenCai OpenAPI key is stored in the pipeline Keychain and must never appear in source, receipts, argv, logs, or shell profiles.

> **2026-08-04 closure amendment (current):** The owner cancelled the recurring five-trading-day automation and explicitly replaced that time gate with one final whole-repository review, a verified local commit, and a clean worktree. The acceptance 1.3 implementation and runbook remain available for future explicitly authorized manual use, but they are no longer scheduled and do not block closure of this Goal. No historical receipt is rewritten or promoted by this waiver.

> **2026-07-30 evidence-boundary hardening:** Provider acceptance metrics count only `origin=live`; controlled injected and live attempts are reported separately. Diagnostics and all cases share the bounded smoke deadline, route drift cannot load an extra live provider, and the CLI returns nonzero on a failed gate after retaining the receipt. An unpublished 1.2 receipt is ignored only after strict file-mode, date, integrity, actual smoke hash and forbidden-field checks; malformed history fails closed. The runbook scans the final acceptance receipt as well as its inputs.

> **2026-07-30 direct-capability hardening:** Direct cases pass only on non-empty `success`; `degraded` and 语义合法 empty remain source-gate failures even when a live success attempt is present. TDX report/notice and Wind filings use a stable 365-day lookback, with Wind still capped at `max_pages=1`, to reduce quiet-period false negatives without adding provider calls.
